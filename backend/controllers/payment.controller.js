...
export const createCheckoutSession = async (req, res) => {
  try {
    ...
    const lineItems = products.map(product => {
      const amount = Math.round(product.price * 100); //send in the format of cents to stripe
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `${configarations.CLIENT_URL}/purchase-success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${configarations.CLIENT_URL}/purchase-cancel`,
      discounts: coupon ? [
