import { UserPlus, Lock, ShoppingCart, LogOut, LogIn } from 'lucide-react'
      ...
      <div className='container mx-auto px-4 py-3'>
        <div className='flex flex-wrap justify-between items-center'>
          ...
          {isAdmin && (
            ...
            className='bg-emerald-700 py-1 px-3 rounded-md font-medium hover:bg-emerald-600 transition duration-300 ease-in-out flex items-center'>
            <Lock className='inline-block mr-1' size={18}/> <span className='hidden sm:inline'>Dashboard</span> 
            </Link>
          )}

          {user ? (
            <button className='bg-gray-700 py-2 px-4 rounded-md hover:bg-gray-600 transition duration-300 ease-in-out flex items-center'
            onClick={logout}
            >
              <LogOut size={18} /><span className='hidden sm:inline ml-2'>Log Out</span></button>
          ) : (
          <>
